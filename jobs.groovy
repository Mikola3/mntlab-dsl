...
job("MNTLAB-${StName}-main-build-job") {
    ...
    steps {
        ...
        shell('chmod +x script.sh && ./script.sh >> output.log && tar -czf child${i}-\${BRANCH_NAME}-\${BUILD_NUMBER}_dsl_script.tar.gz output.log')
    }
    publishers { 
...
job("MNTLAB-${StName}-child${i}-build-job") {
    ...
    steps {
        shell('chmod +x script.sh && ./script.sh >> output.log && tar -czf child${i}-\${BRANCH_NAME}-\${BUILD_NUMBER}_dsl_script.tar.gz output.log jobs.groovy script.sh')
    }
    publishers { 
        archiveArtifacts {
            pattern('output.log')
