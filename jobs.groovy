...
job("MNTLAB-alahutsin-child1-build-job") {
    ...
    steps {
      shell('chmod +x do.sh && ./do.sh > output.log && pwd && tar -czf child1_${BUILD_NUMBER}_dsl_do.tar.gz output.log jobs.groovy do.sh')
    }
    publishers { 
        archiveArtifacts {
            pattern('output.log')
...
job("MNTLAB-alahutsin-child2-build-job") {
    ...
    steps {
      shell('chmod +x do.sh && ./do.sh > output.log && pwd && tar -czf child2_${BUILD_NUMBER}_dsl_do.tar.gz output.log jobs.groovy do.sh')
    }
    publishers { 
        archiveArtifacts {
            pattern('output.log')
  ...
  job("MNTLAB-alahutsin-child3-build-job") {
    ...
    steps {
      shell('chmod +x do.sh && ./do.sh > output.log && pwd && tar -czf child3_${BUILD_NUMBER}_dsl_do.tar.gz output.log jobs.groovy do.sh')
    }
    publishers { 
        archiveArtifacts {
            pattern('output.log')
